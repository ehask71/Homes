...
class AccountController extends AppController {
    ...
    public function professionals_billingprofile() {
        if ($this->Auth->user('authnet_profile') != 0) {
            ...
            if (!$cimresponse->isError()) {
                ...
                $this->Session->setFlash(__('No Billing Profiles Found!'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-error'
                ));
            ...
            // No Billing Profile
            $this->Session->setFlash(__('Please Create A Billing Profile'), 'alert', array(
                'plugin' => 'BoostCake',
            ));
    ...
    public function professionals_editbilling($id) {
        if ($id != '') {
            ...
            $this->Session->setFlash(__('Missing Billing Profile'), 'alert', array(
                'plugin' => 'BoostCake',
                'class' => 'alert-error'
            ));
            $this->redirect('/professionals/account/');
    ...
    public function professionals_createbilling() {
        if ($this->Auth->user('authnet_profile') != 0) {
            $this->Session->setFlash(__('Profile Exists!'), 'alert', array(
                'plugin' => 'BoostCake',
                'class' => 'alert-error'
            ));
        ...
        if ($this->request->is('post')) {
            ...
            if (!$cimresponse->isError()) {
                ...
                if ($this->Account->save($update)) {
                    ...
                    $this->Session->setFlash(__('Billing Profile Created'), 'alert', array(
                        'plugin' => 'BoostCake',
                        'class' => 'alert-success'
                    ));
                    $this->redirect('/professionals/account/');
                ...
                $this->Session->setFlash(__('Error Creating The Billing Profile'), 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-error'
                ));
    ...
    public function login() {
        if ($this->request->is('post')) {
            if ($this->Auth->login()) {
                ...
                $this->Session->setFlash('Your username or password was incorrect.', 'alert', array(
                    'plugin' => 'BoostCake',
                    'class' => 'alert-error'
                ));
            }
